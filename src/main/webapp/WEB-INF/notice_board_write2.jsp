  ...
  >
    ...
    <div class="flex items-center gap-3 2xsm:gap-7">
      ...
      <main>
		...
		<input type="hidden" name="uploadedImageUrl" id="uploadedImageUrl" value="">
		              ...
		              <tr>				 
						  <td colspan="3">
						    <div>
						      ...
						      <textarea name="n_content" id="n_content" rows="6" placeholder="내용을 입력하세요"></textarea>
						      <script>
								ClassicEditor
								  ...
								  .then(editor => {
								        ...
								        upload: function() {
								          return loader.file
								            .then(function (file) {
								              ...
								              .then(data => {
								                if(data.uploaded) {
								                  $('#uploadedImageUrl').val(data.urls.join(','));  // URL을 hidden input에 저장
								                  return { default: data.urls[0] };  // 첫 번째 이미지 URL을 에디터에 삽입
								                }
								              });
								            });
								        }
								    ...
								    const editorContainer = editor.ui.getEditableElement().parentElement;
								    editorContainer.classList.add('w-full', 'rounded-lg', 'border-[1.5px]', 'border-stroke', 'bg-transparent', 'py-3', 'px-5', 'font-medium', 'outline-none', 'transition', 'focus:border-primary', 'active:border-primary', 'disabled:cursor-default', 'disabled:bg-whiter', 'dark:border-form-strokedark', 'dark:bg-form-input', 'dark:focus:border-primary');
								  })
								  .catch(error => {
								    console.error(error);
								  });
